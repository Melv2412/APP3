# Utilité de Chaque Phase dans ASIKO

## Vue d'ensemble

Ce document détaille l'utilité et le rôle de chaque phase dans le projet ASIKO (Système Prédictif pour Anticiper la Pneumonie).
...
- Déclenchement automatique (signals Django)
...
**Note** : Cette phase fournit des **APIs JSON** (pas d'interface utilisateur). Le front Flutter consommera ces APIs pour afficher les visualisations.
...
**Note** : Cette phase fournit des **APIs JSON** qui agrègent toutes les données. Le front Flutter affichera l'historique.
...
- ❌ **Vous ne créez PAS** : Interface utilisateur (c'est le front Flutter)
...
4. Phase 3 : Sensors + Predictions IA ⚠️
