"""
Script pour créer des données de test pour les établissements de santé d'Abidjan.
Usage: python manage.py shell < create_facilities_data.py
"""
...
for data in facilities_data:
    ...
    if created:
        created_count += 1
        print(f"✅ Créé: {facility.name}")
    else:
        print(f"⚠️  Existe déjà: {facility.name}")

print(f"\n🎉 {created_count} établissement(s) créé(s) sur {len(facilities_data)} au total.")
print(f"📊 Total dans la base: {HealthFacility.objects.count()}")
